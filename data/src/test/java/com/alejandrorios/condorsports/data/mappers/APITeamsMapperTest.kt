package com.alejandrorios.condorsports.data.mappers

import com.alejandrorios.condorsports.data.base.MockableTest
 ...
 */
class APITeamsMapperTest : APITeamsMapperDependentTest() {

    @Test
    fun shouldBeAbletoMapFromApiTeams() {
        val team = APITeamsMapper.map(apiTeam)
        val apiTeam = APITeamsMapper.map(team)

        Assert.assertNotNull(team)
        Assert.assertEquals(team.idTeam, apiTeam.idTeam)
        Assert.assertEquals(team.strTeam, apiTeam.strTeam)
        Assert.assertEquals(team.strAlternate, apiTeam.strAlternate)
        Assert.assertEquals(team.intFormedYear, apiTeam.intFormedYear)
        Assert.assertEquals(team.strStadium, apiTeam.strStadium)
        Assert.assertEquals(team.strWebsite, apiTeam.strWebsite)
        Assert.assertEquals(team.strFacebook, apiTeam.strFacebook)
        Assert.assertEquals(team.strTwitter, apiTeam.strTwitter)
        Assert.assertEquals(team.strInstagram, apiTeam.strInstagram)
        Assert.assertEquals(team.strDescriptionEN, apiTeam.strDescriptionEN)
        Assert.assertEquals(team.strTeamBadge, apiTeam.strTeamBadge)
        Assert.assertEquals(team.strTeamJersey, apiTeam.strTeamJersey)
        Assert.assertEquals(team.strYoutube, apiTeam.strYoutube)
    }
}

abstract class APITeamsMapperDependentTest : MockableTest {

    protected var apiTeam = APITeams(
        "133604",
        "Arsenal",
        "Gunners",
        "1892",
